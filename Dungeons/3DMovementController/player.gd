...
func _physics_process(delta):
	...
	# Process input only when not moving or rotating
	if not moving and not rotating:
		...
		if input_dir != 0 or slide_input_dir != 0:
			...
			if not collider:
				movement_target = global_position + (transform.basis.z * input_dir if input_dir != 0 else transform.basis.x * slide_input_dir)
				moving = true
